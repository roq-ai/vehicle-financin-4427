...
const notificationMapping: Record<string, NotificationConfigInterface> = {
  'loan_application_submitted.create': {
    roles: ['business-owner', 'loan-officer'],
  ...
  'loan_application_rejected.update': {
    roles: ['business-owner'],
    key: 'loan_application_id',
    tenantPath: [],
    userPath: [],
...
const tenantRoles: string[] = ['business-owner', 'loan-officer'];
...
export async function notificationHandlerMiddleware(req: NextApiRequest, entityId: string) {
  ...
  const tenant = await prisma.lender.findFirst({
