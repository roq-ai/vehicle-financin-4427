...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['business-owner', 'loan-officer'];
  ...
  const { data, error, isLoading } = useSWR<LenderInterface[]>(
    ...
    () => getLenders({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    { name: 'Loans', path: '/loans', entity: 'loan', service: AccessServiceEnum.PROJECT, icon: FiCreditCard },
    {
      name: 'Loan Officers',
      path: '/loan-officers',
      entity: 'loan_officer',
